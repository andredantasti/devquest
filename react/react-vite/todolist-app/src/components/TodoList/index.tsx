...
const TodoList = ({
  ...
  return (
    <>
          ...
          {todoList.map((todo) => (
            ...
            >
              <div className="flex items-center gap-4">
                <span className="w-6 h-6 rounded-full hover:bg-[linear-gradient(to_right,hsl(192,100%,67%),hsl(280,87%,65%))] hover:p-[2px]">
                  <button
                    onClick={() => toggleTodoCompleted(todo.id)}
                    className={`w-full h-full border rounded-full ${
                      themeConfig[theme].todo.borderColor
                    } cursor-pointer ${themeConfig[theme].todo.backgroundColor}
                    ${
                      todo.completed
      ...
      <div
        className={`flex justify-center gap-5 py-4 rounded-md mt-4 ${themeConfig[theme].todo.backgroundColor} ${themeConfig[theme].layout.textColor} sm:hidden`}
      >
        <button
          onClick={() => setFilter("all")}
          className={`${
